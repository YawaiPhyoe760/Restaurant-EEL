pages = ['homePage', 'tablePage', 'dishPage', 'orderPage', 'createTablePage', 'editTablePage',
    'createDishPage', 'editDishPage', 'createOrderPage'];
...
create_order_btn.addEventListener('click', function (event) {
    ...
    let order_count = document.querySelector("#create_order_count").value;

    eel.createOrder(dish_id, table_id, order_count)(function (tableId) {
...
function loadDish() {
    eel.getDishes()(function (data) {
        dishStr = '';
        data.forEach((dish) => {
            let cc = dish.status == 1 ? 'success' : 'secondary';
            dishStr += `
                <tr>
                    <td>${dish.id}</td>
                    <td>${dish.name}</td>
                    <td>${dish.price}</td>
                    <td><button class="btn btn-${cc} btn-sm" onclick="dishStatusChange(${dish.status},${dish.id})">${dish.status}</button></td>
                    <td>
                        <button class="btn btn-warning btn-sm" onclick="editDish(${dish.id})"><i class="fa fa-edit"></i></button>
